...
public class UidapterBoardController {
	...
	public NexacroResult selectItemList() throws NexacroException{
			...
			ArrayList<Map<String,Object>> ds_itemCombo = new ArrayList<Map<String,Object>>(); // return값 ds_itemCombo 객체
			
			ds_itemCombo = uidapterSampleService.selectItemList(); // 요청 처리를 Service로 이관
			
			result.addDataSet("ds_itemCombo", ds_itemCombo); // return값 세팅
			System.out.println("주문상품 :" + ds_itemCombo);
			return result; // 회신
	}
	
	@RequestMapping(value = "/orderBoard/selectOrdList.do")
	public NexacroResult selectOrdList(@ParamDataSet(name="ds_searchList",required=false) Map<String,Object> ds_searchList) throws NexacroException{
		
		NexacroResult result = new NexacroResult(); // return 값을 세팅하기 위한 객체 생성
		ArrayList<Map<String,Object>> ds_list = new ArrayList<Map<String,Object>>(); // return 값 ds_list 객체 생성
		
		ds_list = uidapterSampleService.selectOrdList(ds_searchList); // 요청 처리를 Service로 이관
		
		result.addDataSet("ds_list", ds_list); // return 값 (key, value)세팅
		return result; // 회신
	...
	public NexacroResult selectCommonCode(@ParamDataSet(name="ds_search") Map<String,Object> ds_search) throws NexacroException{
		 System.out.println("요청 값 :" + ds_search);
		NexacroResult result = new NexacroResult(); // return값을 세팅하기 위한 객체 생성
		ArrayList<Map<String,Object>> ds_commonCode = new ArrayList<Map<String,Object>>(); // return값 ds_ordStatCombo 객체 생성
	
		ds_commonCode = uidapterSampleService.selectCommonCode(ds_search); // 요청처리를 Service로 이관	
																		// Service로부터 받아온 결과값을 ds_commonCode에 삽입

		result.addDataSet("ds_commonCode", ds_commonCode); // return값 세팅
	   System.out.println("회신 값 :" + result);
		return result; // 회신
	}
